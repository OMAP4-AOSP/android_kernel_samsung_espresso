...
/**
 * struct twl6025_resource_map - describe the resource mapping for TWL6025
 * @name:	name of the resource
 * @res_id:	resource ID
...
/* list of all s/w modifiable resources in TWL6025 */
static struct twl6025_resource_map twl6025_res_map[] = {
/* PREQx_RES_ASS_A register resources */
TWL6025_RES_DATA(RES_LDOUSB, "VUSB", PREQ_RES_ASS_A, DEV_GRP_P1, BIT(5)),
TWL6025_RES_DATA(RES_SMPS5, "SMPS5", PREQ_RES_ASS_A, DEV_GRP_P1, BIT(4)),
TWL6025_RES_DATA(RES_SMPS4, "SMPS4", PREQ_RES_ASS_A, DEV_GRP_P1, BIT(3)),
TWL6025_RES_DATA(RES_SMPS3, "SMPS3", PREQ_RES_ASS_A, DEV_GRP_P1, BIT(2)),
TWL6025_RES_DATA(RES_SMPS2, "SMPS2", PREQ_RES_ASS_A, DEV_GRP_P1, BIT(1)),
TWL6025_RES_DATA(RES_SMPS1, "SMPS1", PREQ_RES_ASS_A, DEV_GRP_P1, BIT(0)),
/* PREQx_RES_ASS_B register resources */
TWL6025_RES_DATA(RES_LDOLN, "LDOLN", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(7)),
TWL6025_RES_DATA(RES_LDO7, "LDO7", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(6)),
TWL6025_RES_DATA(RES_LDO6, "LDO6", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(5)),
TWL6025_RES_DATA(RES_LDO5, "LDO5", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(4)),
TWL6025_RES_DATA(RES_LDO4, "LDO4", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(3)),
TWL6025_RES_DATA(RES_LDO3, "LDO3", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(2)),
TWL6025_RES_DATA(RES_LDO2, "LDO2", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(1)),
TWL6025_RES_DATA(RES_LDO1, "LDO1", PREQ_RES_ASS_B, DEV_GRP_P1, BIT(0)),
/* PREQx_RES_ASS_C register resources */
TWL6025_RES_DATA(RES_VSYSMIN_HI, "VSYSMIN_HI", PREQ_RES_ASS_C, DEV_GRP_P1, BIT(5)),
TWL6025_RES_DATA(RES_32KCLKG, "32KCLKG", PREQ_RES_ASS_C, DEV_GRP_P1, BIT(4)),
TWL6025_RES_DATA(RES_32KCLKAUDIO, "32KCLKAUDIO", PREQ_RES_ASS_C, DEV_GRP_P1, BIT(3)),
TWL6025_RES_DATA(RES_SYSEN, "SYSEN", PREQ_RES_ASS_C, DEV_GRP_P1, BIT(2)),
TWL6025_RES_DATA(RES_REGEN2, "REGEN2", PREQ_RES_ASS_C, DEV_GRP_P1, BIT(1)),
TWL6025_RES_DATA(RES_REGEN, "REGEN1", PREQ_RES_ASS_C, DEV_GRP_P1, BIT(0)),
};
...
static void twl6030_process_system_config(void)
{
	u8 grp;
	int r;
...
static void twl6030_program_map(void)
{
...
static void twl6025_program_map(void)
{
	struct twl6025_resource_map *res = twl6025_res_map;
	int r, i;
	...
	for (i = 0; i < ARRAY_SIZE(twl6025_res_map); i++) {
		for (grp = 0; grp < DEV_GRP_CNT; grp++)
			/* map back from generic device id to TWL6025 mask */
	...
	r = twl_i2c_write(TWL6030_MODULE_ID0, &val[0],
		TWL6025_PREQ1_RES_ASS_A, PREQ_RES_ASS_REG_CNT);
	if (r)
		pr_err("%s: Error(%d) programming TWL6025 PREQ Assignment Registers {start addr=0x%02x}\n",
		       __func__, r, TWL6025_PREQ1_RES_ASS_A);
}

static void twl6030_update_system_map
...
static void twl6030_update_map(struct twl4030_resconfig *res_list)
{
	int i, res_idx = 0;
...
static void twl6025_update_map(struct twl4030_resconfig *res_list)
{
	int i, res_idx = 0;
	struct twl6025_resource_map *res;
	...
	while (res_list->resource != TWL4030_RESCONFIG_UNDEF) {
		...
		if (i == ARRAY_SIZE(twl6025_res_map)) {
 ...
 */
void twl6030_power_init(struct twl4030_power_data *power_data,
		unsigned long features)
{
	...
	if (power_data && power_data->resource_config) {
		if (features & TWL6025_SUBCLASS)
			twl6025_update_map(power_data->resource_config);
		else
